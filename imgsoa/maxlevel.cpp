...
ImageSOA<U> maxlevel(ImageSOA<T> inputImage, int const oldLevel, int const newLevel) {
  ...
  for (size_t i = 0; i < inputImage.r.size(); ++i) {
    outputImage.r[i] = static_cast<U>(inputImage.r[i] * transformation);
    outputImage.g[i] = static_cast<U>(inputImage.g[i] * transformation);
    outputImage.b[i] = static_cast<U>(inputImage.b[i] * transformation);
  }
  return outputImage;
}
